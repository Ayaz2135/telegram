...
async def show_account_info(query):
    ...
    message += f"📅 **Registered:** {user['created_at'][:10]}\n"
    message += f"🔐 **Last Login:** {user['last_login'][:10] if user['last_login'] else 'N/A'}\n\n"
...
async def manage_ad(query, ad_id: int):
    ...
    text += f"**Created:** {ad['created_at'][:10]}\n\n"
...
async def run_bot():
    ...
    try:
        ...
        # Create application with minimal configuration to avoid Updater issues
        ...
        try:
            # Alternative approach - build step by step
            from telegram.ext import ApplicationBuilder
            builder = ApplicationBuilder().token(AD_BOT_TOKEN)
            application = builder.build()
            print("Alternative application creation successful!")
...
def main():
    ...
    # Run bot in main thread with proper event loop handling
    try:
        ...
        if "there is no current event loop in thread" in str(e):
            # Handle case where event loop is already running or not available
            try:
                print("Getting current event loop...")
                loop = asyncio.get_event_loop()
            except RuntimeError:
                print("Creating new event loop...")
                loop = asyncio.new_event_loop()
                asyncio.set_event_loop(loop)
            print("Running bot with loop.run_until_complete...")
            loop.run_until_complete(run_bot())
        else:
            # Re-raise if it's a different RuntimeError
            print("Re-raising different RuntimeError...")
            raise
    except Exception as e:
